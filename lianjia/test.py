...
def get_sc(i):
    time.sleep(shui)
    full_url= url+ houseid[i]
    print(full_url)
    sc = requests.get(url, headers=my_head).content.decode('UTF-8')      # 调用get_html 获取网页源码
    return sc
...
def get_detail(page):
    ...
    for z in range(len(houseid)):
        ...
        title = check_none('//title/text()', selector, sc)

        price = check_none('//div[5]/div[2]/div[3]/span[1]/text()', selector, sc)
        danwei = check_none('//div[5]/div[2]/div[3]/span[2]/span/text()', selector, sc)
        sumprice = price+danwei
        name = check_none('//div[5]/div[2]/div[5]/div[1]/a[1]/text()', selector, sc)
        district = check_none('//div[5]/div[2]/div[5]/div[2]/span[2]/a[1]/text()', selector, sc)
